...
public class FindPrimesTask extends MultithreadedTask implements FPT{
    ...
    private class BruteForceTask extends MultithreadedTask.SubTask{
        ...
        private void dispatchPrimeFound(final long number){
            ...
            // Write to cache if the list is larger than the buffer size
            if (0 < bufferSize && bufferSize <= primes.size()){
        ...
        private void writeToCache(){
            final File file = FileManager.getInstance().getTaskCacheDirectory(FindPrimesTask.this);
            try (final DataOutputStream dataOutputStream = new DataOutputStream(new FileOutputStream(file, true))){
